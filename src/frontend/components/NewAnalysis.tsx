...
const Step1 = ({ onFinish, onBack, hideHeader }: { onFinish: (listing: ScrapedListing) => void, onBack: () => void, hideHeader?: boolean }) => {
    ...
    const handleFetch = async () => {
        ...
        if (!trimmed.includes('hemnet.se')) {
            setError('Länken måste vara från hemnet.se');
            return;
        }
    ...
    // Extract address preview from URL for loading state
    const urlPreview = (() => {
        try {
            const parts = url.split('/');
            const slug = parts[parts.length - 1] || parts[parts.length - 2] || '';
            // Remove trailing ID and clean up
            return slug.replace(/-\d+$/, '').replace(/-/g, ' ').replace(/^\w/, c => c.toUpperCase()) || 'Hämtar...';
        } catch { return 'Hämtar...'; }
    })();
                ...
                <section className="bg-white dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark p-5 space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[12px] text-text-muted leading-snug">Klistra in en Hemnet-länk för att starta</p>
                        </div>
                    </div>
                    <div className="relative">
                        <input
                            ...
                            type="url"
                            value={url}
                            onChange={(e) => { setUrl(e.target.value); setError(''); setFetchState('idle'); }}
                        />
                    ...
                    {fetchState !== 'loading' && (
                        <button
                            onClick={handleFetch}
                            disabled={!url.trim()}
                            className="w-full h-[56px] bg-primary text-white font-semibold text-[15px] rounded-2xl flex items-center justify-center gap-3 active:scale-[0.98] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                    ...
                    <section className="bg-white dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark p-5 animate-slide-up">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[12px] text-text-muted">{urlPreview}</p>
                            </div>
                        </div>
                    </section>
                )}
...
const Step2 = ({ listing, onNext, onBack, hideHeader }: { listing: ScrapedListing, onNext: () => void, onBack: () => void, hideHeader?: boolean }) => {
    ...
    const shortUrl = (() => {
        try {
            const u = new URL(listing.hemnetUrl);
            const path = u.pathname;
            return `hemnet.se${path.length > 35 ? path.slice(0, 32) + '...' : path}`;
        } catch { return listing.hemnetUrl; }
    })();
    ...
    return (
        <div className="flex-1 flex flex-col h-full relative bg-page-bg-light dark:bg-page-bg-dark">
            ...
            <main className="flex-1 px-page-x py-6 space-y-6 overflow-y-auto hide-scrollbar">
                <section>
                    <div className="relative">
                        <input className="w-full bg-surface-input dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-2xl px-5 py-4 text-[13px] text-text-secondary dark:text-white/60 font-medium focus:ring-0 outline-none" readOnly type="text" value={shortUrl} />
                ...
                <section className="bg-white dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark overflow-hidden">
                    <div className="p-5 space-y-6">
                        <div>
                            ...
                            <p className="text-[14px] text-text-secondary mt-1 font-medium">{listing.address}{listing.area ? `, ${listing.area}` : ''}</p>
                        </div>

                        {/* Scraped details summary */}
                        <div className="grid grid-cols-2 gap-3">
                            {listing.price && (
                                <div className="bg-surface-input dark:bg-surface-dark rounded-xl p-3">
                                    ...
                                    <p className="text-[14px] font-bold text-text-main dark:text-white">{listing.price}</p>
                                </div>
                            )}
                            {listing.avgift && (
                                <div className="bg-surface-input dark:bg-surface-dark rounded-xl p-3">
                                    <p className="text-[10px] text-text-muted font-semibold uppercase tracking-widest mb-0.5">Avgift</p>
                                    <p className="text-[14px] font-bold text-text-main dark:text-white">{listing.avgift}</p>
                                </div>
                            )}
                            {listing.sqm && (
                                <div className="bg-surface-input dark:bg-surface-dark rounded-xl p-3">
                                    <p className="text-[10px] text-text-muted font-semibold uppercase tracking-widest mb-0.5">Storlek</p>
                                    <p className="text-[14px] font-bold text-text-main dark:text-white">{listing.sqm}</p>
                                </div>
                            )}
                            {listing.rooms && (
                                <div className="bg-surface-input dark:bg-surface-dark rounded-xl p-3">
                                    <p className="text-[10px] text-text-muted font-semibold uppercase tracking-widest mb-0.5">Rum</p>
                                    <p className="text-[14px] font-bold text-text-main dark:text-white">{listing.rooms}</p>
                                </div>
                            )}
                        </div>
...
const Step3 = ({ listing, onNext, onBack, hideHeader }: { listing: ScrapedListing, onNext: () => void, onBack: () => void, hideHeader?: boolean }) => {
    // Generate a valuation range based on scraped price
    const valuationLow = listing.priceRaw ? Math.round(listing.priceRaw * 1.02) : 0;
    const valuationHigh = listing.priceRaw ? Math.round(listing.priceRaw * 1.08) : 0;
                ...
                <section className="bg-white dark:bg-surface-dark rounded-2xl overflow-hidden border border-border-light dark:border-border-dark">
                    {listing.imageUrl && (
                        <div className="aspect-[16/9] bg-surface-input dark:bg-surface-dark relative overflow-hidden">
                            <img alt="Property preview" className="w-full h-full object-cover grayscale opacity-90" src={listing.imageUrl} />
                    ...
                    <div className="p-5">
                        <div className="flex justify-between items-start mb-4">
                            <div className="space-y-0.5">
                                <h2 className="text-xl font-bold tracking-tight text-text-main dark:text-white">{listing.address}</h2>
                                <p className="text-[13px] text-text-muted font-medium">{listing.area}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] uppercase tracking-widest text-text-muted font-semibold mb-0.5">Pris</p>
                                <p className="text-lg font-bold text-text-main dark:text-white">{listing.price}</p>
                            </div>
                        </div>
                        {/* Details row */}
                        {(listing.rooms || listing.sqm || listing.avgift) && (
                            <div className="flex items-center gap-2 text-[12px] text-text-muted font-medium mb-3 flex-wrap">
                                {listing.rooms && <span>{listing.rooms}</span>}
                                {listing.rooms && listing.sqm && <span>·</span>}
                                {listing.sqm && <span>{listing.sqm}</span>}
                                {(listing.rooms || listing.sqm) && listing.avgift && <span>·</span>}
                                {listing.avgift && <span>{listing.avgift}</span>}
                            </div>
                        )}
                        {/* Green badge */}
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="bg-white dark:bg-surface-dark p-5 rounded-2xl border border-border-light dark:border-border-dark">
                            <div className="flex justify-between items-baseline mb-5">
                                ...
                                <span className="text-2xl font-bold text-text-main dark:text-white tracking-tighter">{formatPrice(valuationLow)} – {formatPrice(valuationHigh)} <span className="text-sm font-semibold text-text-muted">SEK</span></span>
                            </div>
...
const Step4 = ({ listing, onFinish, onBack }: { listing: ScrapedListing, onFinish: () => void, onBack: () => void }) => (
            ...
            <div className="bg-white rounded-t-[24px] px-6 pt-6 pb-12 shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-start border-b border-gray-100 pb-4">
                        <div className="flex flex-col">
                            <span className="text-[15px] font-medium text-black">Analys: {listing.address}</span>
                        </div>
...
export const NewAnalysis = ({ onFinish, onBack, hideHeader }: { onFinish: () => void, onBack: () => void, hideHeader?: boolean }) => {
    ...
    const handleStep1Finish = (scraped: ScrapedListing) => {
        setListing(scraped);
        setStep(2);
    };
    ...
    return (
        <div className={`w-full h-full bg-page-bg-light dark:bg-page-bg-dark text-text-main dark:text-white relative ${hideHeader ? 'hide-inner-headers' : ''}`}>
            {step === 1 && <Step1 onFinish={handleStep1Finish} onBack={onBack} hideHeader={hideHeader} />}
            {step === 2 && listing && <Step2 listing={listing} onNext={() => setStep(3)} onBack={prevStep} hideHeader={hideHeader} />}
            {step === 3 && listing && <Step3 listing={listing} onNext={() => setStep(4)} onBack={prevStep} hideHeader={hideHeader} />}
            {step === 4 && listing && <Step4 listing={listing} onFinish={onFinish} onBack={prevStep} />}
        </div>
    );
};
